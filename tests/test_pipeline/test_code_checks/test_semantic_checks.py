...
class TestSyntaxCheckPHP:
    def test_valid_php_code(self, mocker):
        ...
        mocker.patch(
            'src.pipeline.code_checks.syntax_checks.php.subprocess_run',
            return_value=mock_result,
        )
        mocker.patch(
            'src.pipeline.code_checks.syntax_checks.php.settings.job_dir',
            '/tmp',
        )
        result = syntax_check_php(code)
